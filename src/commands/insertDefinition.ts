...
async function definitionFileUriForDocument(
  term: string,
  ...
  const defsDir = vscode.Uri.joinPath(
    workspaceFolder.uri,
    "web",
    "static",
    "definitions"
...
async function ensureDefinitionFile(
  term: string,
  fileUri: vscode.Uri
): Promise<void> {
  try {
...
async function fetchWikipediaSummary(term: string): Promise<string> {
  ...
  return new Promise<string>((resolve) => {
    const req = https.get(
      apiUrl,
      {
        headers: { accept: "application/json", "user-agent": "ixdar-vs/0.0.1" },
      },
      (res: any) => {
        let body = "";
...
): { range: vscode.Range; word: string } | undefined {
...
async function replaceWithShortcode(
  editor: vscode.TextEditor,
  range: vscode.Range,
  term: string
): Promise<void> {
...
async function replaceAllOccurrencesInDocument(
  document: vscode.TextDocument,
  word: string
): Promise<number> {
...
export async function insertDefinitionForWord(
  word: string,
  filePath: string
  ...
  try {
    ...
    if (!fileUri) {
      return {
        success: false,
        message: "Unable to resolve workspace folder for definitions.",
      };
    }
    ...
    if (replacements === 0) {
      return {
        success: true,
        message: `No occurrences of "${term}" found in the document.`,
        replacements: 0,
      };
    }
...
const commandFunc = async () => {
  ...
  if (!fileUri) {
    vscode.window.showErrorMessage(
      "Unable to resolve workspace folder for definitions."
    );
    return;
  }
  await ensureDefinitionFile(term, fileUri);
...
const mcpFunc = async (args: any): Promise<McpResult> => {
  ...
  if (!result.success) {
    return mcp.returnMcpError({ error: result.message });
  }
...
const description =
  "Insert a definition shortcode in a Markdown file. Replaces all occurrences of the specified word with a Hugo definition shortcode. Fetches Wikipedia summary and creates definition file.";
...
const inputSchema = {
  ...
  properties: {
    word: {
      type: "string",
      description:
        "The word to create a definition for and replace in the document",
    },
    filePath: {
      type: "string",
